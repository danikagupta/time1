...
st.set_page_config(page_title="California Map", page_icon=":guardsman:", layout="wide")

# Create a map centered on California
california_map = folium.Map(location=[37.0902, -119.4179], zoom_start=6)

# Add markers for Palo Alto and Irvine
palo_alto = folium.Marker(location=[37.4418, -122.1430], popup="Palo Alto", icon=folium.Icon(color='blue'))
palo_alto.add_child(folium.Popup("<a href='https://www.cityofpaloalto.org/' target='_blank'>Visit the City of Palo "
                                 "Alto website</a>"))
palo_alto.add_to(california_map)

irvine = folium.Marker(location=[33.6845, -117.8265], popup="Irvine", icon=folium.Icon(color='green'))
irvine.add_child(folium.Popup("<a href='https://www.cityofirvine.org/' target='_blank'>Visit the City of Irvine "
                              "website</a>"))
irvine.add_to(california_map)

st_data = streamlit_folium.st_folium(california_map, width=725)

california_city_data = [
    ['Berkeley', 37.8715, 122.273],
    ['Hayward', 37.6688, 122.081],
    ['Livermore', 37.6819, 121.7685],
    ['Mountain_View', 37.3861, 122.0839],
    ['Richmond', 37.9358, 122.3477],
    ['San_Jose', 37.3387, 121.8853],
    ['San_Mateo', 37.563, 122.3255],
    ['San_Ramon', 37.7644, 121.954],
    ['South_San_Francisco', 37.6547, 122.4077],
